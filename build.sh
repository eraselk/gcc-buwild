...
strip_binaries() {
    ...
    for TARGET in ${TARGETS[@]}; do
        case $TARGET in
        x86_64*)
            grep "x86-64" .file-idx | grep "not strip" | tr ':' ' ' | awk '{print $1}' | while
                read -r file
            do
                strip -s "$file"
            done
        ...
        aarch64*)
            ...
            grep "ARM" .file-idx | grep "aarch64" | grep "not strip" | tr ':' ' ' | awk '{print $1}' | while read -r file; do
                ./stripp-a64 -s "$file"
            done
            ;;
        arm*)
            cp -rf $PREFIX/$TARGET/bin/$TARGET-strip ./stripp-a32
            grep "ARM" .file-idx | grep "eabi" | grep "not strip" | tr ':' ' ' | awk '{print $1}' | while read -r file; do
                ./stripp-a32 -s "$file"
            done
            ;;
        esac
...
git_push() {
    ...
    cat README | sed s:GCCVERSION:$FULL_VERSION-$BUILD_DATE:g | sed s:BINUTILSVERSION:$BINUTILS_VERSION:g >README.md
